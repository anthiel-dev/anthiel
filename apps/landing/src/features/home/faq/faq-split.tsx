...
export function FaqSplit() {
  ...
  return (
    <div className="faq-split grid gap-8 sm:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)] sm:gap-10">
      <ul className="m-0 list-none p-0" role="list" data-reveal-item data-stagger={2}>
        {faqs.map((faq) => {
          const isActive = faq.id === activeId;

          return (
            <li key={faq.id} className="border-b border-white/[0.05] last:border-b-0">
              <button
                type="button"
                className={cn(
      ...
      >
        {active ? (
          <div key={active.id} className="faq-answer-enter">
            ...
            <p className="mt-3 font-display text-[22px] leading-snug tracking-tight text-white/90 sm:text-[26px]">
              {active.question}
            </p>
