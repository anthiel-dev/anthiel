...
export function ContactEmailForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await createContactQuery({
        email: value,
        message: contact.internalMessage,
      });
      if (result.ok) {
        setStatus("success");
        setEmail("");
  ...
  if (isDone) {
    return (
      <p className="faq-answer-enter text-sm tracking-tight text-white/80">{contact.success}</p>
    );
  }
    ...
    <div className="w-full">
      ...
      >
        {status === "invalid"
          ? contact.invalid
          : status === "rate-limited"
            ? contact.rateLimited(retryMinutes)
            : status === "error"
              ? contact.error
              : "\u00a0"}
      </p>
